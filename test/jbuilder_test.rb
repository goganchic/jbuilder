...
class JbuilderTest < ActiveSupport::TestCase
  ...
  test "nesting single child with mixed extract" do
    person = Class.new do
      attr_reader :name, :age
    ...
    json = Jbuilder.encode do |json|
      json.persons [person], :name do |json, person|
        json.age person.age
      end
    end

    JSON.parse(json).tap do |parsed|
      assert_equal "David", parsed["persons"][0]["name"]
      assert_equal 32, parsed["persons"][0]["age"]
    end
  end

  test "nesting multiple children with mixed extract" do
    comments = [ Struct.new(:content, :id).new("hello", 1), Struct.new(:content, :id).new("world", 2) ]

    json = Jbuilder.encode do |json|
      json.comments comments, :content do |json, comment|
        json.id comment.id
      end
    end
